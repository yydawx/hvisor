...
impl ArchCpu {
    ...
    fn setup_vmcs_control(&mut self) -> HvResult {
        ...
        info!("[VMX] Secondary controls capability: allowed0={:#x}, allowed1={:#x}", sec_ctl_allowed0, sec_ctl_allowed1);
        info!("[VMX] UNRESTRICTED_GUEST bit in allowed1: {}", (sec_ctl_allowed1 >> 7) & 1);
        ...
        let desired_sec_ctl = (CpuCtrl2::ENABLE_EPT
            ...
            .bits();
        info!("[VMX] Desired secondary controls: {:#x}", desired_sec_ctl);
        ...
        match Vmcs::set_control(
            ...
            Ok(_) => {
                ...
                info!("[VMX] Secondary controls set successfully, actual={:#x}", actual);
            }
            Err(e) => {
    ...
    fn setup_vmcs_guest(&mut self, entry: GuestPhysAddr, rsp: GuestPhysAddr) -> HvResult {
        ...
        info!("[VMCS] setup_vmcs_guest: multiboot_mode={}, entry={:#x}, rsp={:#x}",
              self.multiboot_mode, entry, rsp);
        if self.multiboot_mode {
            //
            // Multiboot2 specifies 32-bit protected mode entry.
            // 64-bit kernels are expected to transition themselves to long mode.
            //
            ...
            // Debug output
            info!("[MULTIBOOT] Setting up 32-bit protected mode (kernel transitions to 64-bit)");
            info!("[MULTIBOOT] CR0_FIXED0={:#x}, CR0_FIXED1={:#x}", cr0_fixed0, cr0_fixed1);
            info!("[MULTIBOOT] CR4_FIXED0={:#x}, CR4_FIXED1={:#x}", cr4_fixed0, cr4_fixed1);
            info!("[MULTIBOOT] Final CR0={:#x}, CR4={:#x}", cr0_guest, cr4_guest);
            ...
            info!("[MULTIBOOT] CS state: sel={:#x}, base={:#x}, limit={:#x}, ar={:#x}",
            ...
            info!("[MULTIBOOT] CS AR: D={}, L={} (should be D=1, L=0 for 32-bit code)", d_bit, l_bit);
            ...
            info!("[MULTIBOOT] SS state: sel={:#x}, ar={:#x} (B bit={} for {}-bit stack)",
            ...
            info!("[MULTIBOOT] VM-entry controls: {:#x}, IA32E_MODE_GUEST bit={}",
                  entry_ctls, (entry_ctls >> 9) & 1);
            info!("[MULTIBOOT] Primary exec controls: {:#x}", proc_ctls);
            info!("[MULTIBOOT] Secondary exec controls: {:#x} (UNRESTRICTED_GUEST={})",
                  proc_ctls2, (proc_ctls2 >> 7) & 1);
            info!("[MULTIBOOT] VM-entry interruption info: {:#x}", intr_info);

            info!("[MULTIBOOT] 32-bit protected mode guest setup: CR0={:#x}, CR4={:#x}, RIP={:#x}",
                  cr0_guest, cr4_guest, entry);
            info!("[MULTIBOOT] Using kernel GDT at 0x80014f0, CS=0x18, DS=SS=0x10");
            info!("[MULTIBOOT] Guest registers: RAX={:#x}, RBX={:#x} (multiboot_info)",
